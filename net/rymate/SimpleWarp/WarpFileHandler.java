...
public class WarpFileHandler {
    ...
    private static SimpleWarpPlugin plugin;
    ...
    public boolean saveSettings() {
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(plugin.m_Folder.getAbsolutePath() + File.separator + "warps.txt"));
            for (Map.Entry entry : plugin.m_warps.entrySet()) {
    ...
    public boolean loadSettings() {
        try {
            BufferedReader reader = new BufferedReader(new FileReader(plugin.m_Folder.getAbsolutePath() + File.separator + "warps.txt"));
            ...
            while (line != null) {
                ...
                if (values.length == 7) {
                    ...
                    if (world != null) {
                        plugin.m_warps.put(values[0], new Location(world, X, Y, Z, yaw, pitch));
                    }
                }
    ...
    public static String[] getList(String name) {
        ...
        try {
            Scanner scanner = new Scanner(new File(plugin.m_Folder.getAbsolutePath() + File.separator + "warps.txt"));
            try {
                int i = 0;
